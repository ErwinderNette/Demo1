...
public class TruckService {
    ...
    public String slowdown(int id){
        truckRepo.getOneTruckById(id);
        changeSpeed(id, -10);
        return "Truck mit der ID "+id +" hat die Geschwidigkeit auf "+ truckRepo.getTruckById(id).getSpeed()+" geändert";
    }
    ...
    // Exit Platoon
    public String exitPlatoon(int id, int temp){
        truckRepo.getTruckList().remove(id);
        truckRepo.getTruckList().remove(temp);
